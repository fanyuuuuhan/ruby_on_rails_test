...
| 🔴 | **禁區** | 教材要求手刻的部分：步驟 8 的 SQL、步驟 17 的查詢 scopes、步驟 22 的認證。可以問觀念，不可以要程式碼 |
...
- 步驟 4-1「用 agent 拼出畫面草稿」：🟢 **這一步是鼓勵你大量使用 agent 的**。`mockup.html` 儘管讓它生、讓它一改再改，直到畫面符合你的想像為止。但畫面清單、版面取捨、列表要顯示哪些資訊，這些判斷要是你的。
- 步驟 4-2 ER 圖：🟡 從畫面反推出來的欄位，要自己一個個驗證過。🔴 **不要讓 AI 直接產 ER 圖或 schema**，那是步驟 6 要跟導師討論的重點。
...
- **步驟 17 是最大的禁區。** 教材明文要求手刻一組 Ransack 風格的查詢 scopes，而且要求你「先用 params 直接串 where 的爛寫法做一次，再重構」。這個「先寫爛的再重構」的過程就是學習本身，AI 一步到位反而毀掉它。
  - 🟢 可以問：「scope 為什麼可以串接」「`sanitize_sql_like` 在防什麼」「index 為什麼能加速查詢、什麼欄位不適合加」。
  - 🔴 不可以問：「幫我寫這些查詢 scope」「幫我寫 `Task.search`」。
